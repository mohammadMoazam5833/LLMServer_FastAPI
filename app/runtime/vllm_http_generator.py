...
class VLLMHttpGenerator:
    ...
    async def generate(
        ...
        **kwargs,
        ...
        logger.info("📤 vLLM non-stream request | model=%s | max_tokens=%d", self.model, max_tokens)
        ...
        try:
            ...
            data = response.json()
            logger.info("✅ vLLM non-stream response received")
            return {
    ...
    async def generate_stream(
        ...
        **kwargs,
        ...
        logger.info("🔗 vLLM stream start | model=%s", self.model)
        chunk_count = 0
        ...
        try:
            async with self._client.stream(
                ...
                async for line in response.aiter_lines():
                    ...
                    raw = line[6:].strip()
                    if raw == "[DONE]":
                        logger.info("🏁 vLLM stream done | chunks=%d", chunk_count)
                        break
                    try:
                        data = json.loads(raw)
                        ...
                        logger.warning("⚠️  JSON parse error on chunk: %s", raw)

        except httpx.RemoteProtocolError:
            logger.error("🚨 vLLM connection closed prematurely")
        except Exception as e:
            logger.error("🚨 vLLM stream exception: %s - %s", type(e).__name__, e)
        finally:
            logger.info("🔌 vLLM stream pipeline closed")
